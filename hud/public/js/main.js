...
function loadAvatar(steamid) {
    ...
    // ORGANIZE PLAYERS
    playersArray = JSON.parse(showGetPlayers()).players;
    ...
    // LOOP THROUGH PLAYERS TO KNOW IF THEY'VE AVATAR
    playersArray.forEach(element => {
    if (sid_new.includes(steamid)) {

        if (element.sid == steamid) {
            if (element.avatar !== undefined) {
                avatar_link = element.avatar.slice(0, -4);
            } else {
                avatar_link = "1";
            }
        }
